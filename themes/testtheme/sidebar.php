...
<div id="sidebar">
    ...
    if($post->post_parent){#when page is a child, aka it has a parent
        wp_list_pages(array('child_of' => $post->post_parent, 'title_li' => __(''))); #show the sub pages without titles
    }else{ #when page does not have a parent
        wp_list_pages(array('child_of' => $post->ID, 'title_li' => __(''))); #show the sub pages without titles
    }
    ...
    <ul class="sub-navigation-items"><?php wp_list_categories(array('title_li' => __(''))); #show categories without titles ?></ul>
    <?php endif; ?>
    </div>
    <!-- End Sub-Navigation -->
